...
export class PagarmeService {
    ...
    static async createOrder(data: {
        ...
        if (data.seller_recipient_id && !data.seller_recipient_id.startsWith('re_test_') && platformRecipientId) {
            payment.split = [
                { amount: Math.round(data.amount * sellerPercentage / 100), recipient_id: data.seller_recipient_id, type: 'percentage', options: { charge_processing_fee: false, charge_remainder_fee: false } },
                { amount: Math.round(data.amount * data.platform_fee_percentage / 100), recipient_id: platformRecipientId, type: 'percentage', options: { charge_processing_fee: true, charge_remainder_fee: true } }
            ];
        }
